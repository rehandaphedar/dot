...
(set-fontset-font "fontset-default" '(#x600 . #x6ff) (font-spec :family "Scheherazade New" :size 28))
...
(use-package org
  ...
  ("C-c o i" . rehandaphedar/org-agenda-inbox)
...
(defun rehandaphedar/org-agenda-config ()
  ...
  (setq org-todo-keywords
		'((sequence "TODO(t)" "NEXT(n)" "QUEUED(q)" "WAIT(w)" "SOMEDAY(s)" "MAYBE(m)" "|" "DONE(d)" "CANC(c)")))
  ...
  (setq rehandaphedar/org-super-agenda-inbox-timegrid-groups
      '(
		(:discard (:not (:tag "inbox")))
		(:name "Today"
			   :and (:time-grid t)
			   :and (:scheduled t :date today)
			   :and (:deadline t :date today))
  ...
  (setq rehandaphedar/org-super-agenda-inbox-groups
		'((:auto-ts t)
		  (:auto-priority t)))
  ...
  (setq org-agenda-custom-commands
		'(("g" "GTD View"
		   ((agenda "" ((org-super-agenda-groups rehandaphedar/org-super-agenda-timegrid-groups)))
			...
			(todo "MAYBE" ((org-agenda-overriding-header "Maybe")))
			(todo "QUEUED" ((org-agenda-overriding-header "Queued")))) "" ("~/Files/roam/agenda.html"))
		  ("r" "Review View"
		   ((todo "WAIT")
			(todo "SOMEDAY")
			(todo "MAYBE")
			(todo "QUEUED")))
		  ("i" "Inbox View"
		   ((agenda "" ((org-super-agenda-groups rehandaphedar/org-super-agenda-inbox-timegrid-groups)))
(tags "+inbox" ((org-super-agenda-groups rehandaphedar/org-super-agenda-inbox-groups)))))))
...
(defun rehandaphedar/org-agenda-inbox ()
  (interactive)
  (org-agenda nil "i"))
...
(use-package org-modern
  ...
  :config
  (set-face-attribute 'org-checkbox nil :font "Font Awesome 7 Free" :inherit 'fixed-pitch))
...
(defun rehandaphedar/presentation-start ()
  ...
  ;; Arabic font
  (set-fontset-font "fontset-default" '(#x600 . #x6ff) (font-spec :family "Scheherazade New" :size 48))
...
(defun rehandaphedar/presentation-end ()
  ...
  (toggle-frame-fullscreen)
  ;; Arabic font
  (set-fontset-font "fontset-default" '(#x600 . #x6ff) (font-spec :family "Scheherazade New" :size 32))
...
(use-package org-similarity
  ...
  (:host github
		 ...
		 :branch "main")
  :init
  (setq org-similarity--findlike-pkg-version "findlike>=2.1.1")
  :config
  (defun org-similarity--deps-available-p ()
    (let (py)
	  (setq py (org-similarity--get-python-interpreter))
      (when (and py (file-exists-p py))
        (eq 0 (call-process py nil nil nil "-c"
                            "import importlib; [importlib.import_module(m) for m in ('findlike','numpy','nltk','sklearn')]")))))
  :custom
  (org-similarity-custom-python-interpreter "~/.local/share/uv/tools/findlike/bin/python")
...
(use-package eglot
  ...
  '((:gopls . (:templateExtensions ["gohtml" "html"])))))
...
(use-package go-mode
  :config
  (add-to-list 'eglot-server-programs '(web-mode . ("gopls"))))
...
(use-package web-mode
  ...
  (add-to-list 'web-mode-engines-alist '("go" . "\\.gohtml\\'")))
...
(use-package toml-mode
  :config
  (setq svelte-basic-offset 4)
  (add-to-list 'eglot-server-programs (cons 'toml-mode '("tombi" "lsp")))
  (add-to-list 'eglot-server-programs (cons 'conf-toml-mode '("tombi" "lsp"))))
...
(use-package emms
  ...
  (setq emms-player-mpv-parameters '("--quiet"
									 "--really-quiet"
									 "--no-audio-display"
									 "--no-video"))
  ...
  ("C-c e h" . (lambda ()
                 (interactive)
                 (emms-stop)
                 (emms-play-playlist "~/Files/audio/hifz/hifz.m3u8")))
  ("C-c e p" . 'emms-pause)
  ("C-c e e" . 'emms)
  ("C-c e s" . 'emms-show))
              ...
              (and (> (buffer-size) 0)
                   (let ((text (buffer-substring-no-properties (point-min) (point-max))))
                     (when (string-suffix-p "\n" text)
                       (put-text-property 0 (length text) 'yank-handler '(evil-yank-line-handler) text))
                     text)))
          (error nil))))
