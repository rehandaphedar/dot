...
backup_dir=/run/media/rehandaphedar/hdd/backups/init/$(date +%F)
mkdir -p $backup_dir

cp -r --parents ~/.gnupg $backup_dir
cp -r --parents ~/.ssh $backup_dir
cp -r --parents ~/.password-store $backup_dir
cp -r --parents ~/Files/mails $backup_dir
cp -r --parents ~/Files/roam $backup_dir
#+end_src

** rehandaphedar Backup Config
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_backup_config
backup_dir=/run/media/rehandaphedar/hdd/backups/config/$(date +%F)
mkdir -p $backup_dir

cp -r --parents ~/.mozilla $backup_dir
cp -r --parents ~/.config/chromium $backup_dir
cp -r --parents ~/.local/share/Anki2 $backup_dir
#+end_src

** rehandaphedar Backup Data
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_backup_data
borg create /run/media/rehandaphedar/hdd/borg::$(date +%F) \
~/Files/art \
~/Files/audio \
~/Files/calls \
~/Files/clients \
