...
x-scheme-handler/mailto=emacsclient-mail-helper.desktop
...
text/csv=libreoffice-calc.desktop;libreoffice-writer.desktop;
...
bind = SUPER, R, global, com.obsproject.Studio:OBSBasic.StartRecording
bind = SUPER, R, global, com.obsproject.Studio:OBSBasic.StopRecording
bind = SUPER+SHIFT, R, global, com.obsproject.Studio:OBSBasic.PauseRecording
bind = SUPER+SHIFT, R, global, com.obsproject.Studio:OBSBasic.UnpauseRecording
...
windowrule = workspace = 1,match:class emacs
...
super + kp1
	chromium --profile-directory="Profile 17"
super + kp2
	chromium --profile-directory="Profile 3"
super + kp3
	chromium --profile-directory="Profile 13"
super + kp4
	chromium --profile-directory="Profile 26"
super + kp5
	chromium --profile-directory="Profile 23"
super + kp6
	chromium --profile-directory="Profile 8"
super + kp7
	chromium --profile-directory="Profile 28"
super + kp8
	chromium --profile-directory="Profile 29"
super + kp9
	chromium --profile-directory="Profile 27"
super + shift + kp1
	chromium --profile-directory="Profile 30"
super + {[,]}
...
#+begin_src conf :tangle ~/.config/swhkd/swhkdrc
super + shift + a
    footclient $SHELL -c 'rehandaphedar_fabric_print_pattern | nvim'
super + a
    footclient $SHELL -c 'rehandaphedar_fabric_run_pattern | nvim'
super + alt + a
    footclient $SHELL -c 'get_links_content | nvim'

super + shift + y
    footclient $SHELL -c 'rehandaphedar_fabric_print_youtube_transcript | nvim'
super + y
    footclient $SHELL -c 'rehandaphedar_fabric_run_youtube_transcript_pattern | nvim'
super + alt + y
    footclient $SHELL -c 'rehandaphedar_fabric_print_youtube_transcript_pattern | nvim'
#+end_src
...
#tray,
#custom-hijri-date, #clock, #custom-go-pray, #pulseaudio, #custom-backlight-keyboard, #custom-backlight-screen, #battery, #privacy, #custom-crypto {
    padding: 0 1rem;
}
...
#custom-backlight-screen, #custom-backlight-keyboard, #battery, #pulseaudio, #privacy, #custom-crypto {
...
* Waybar Crypto
#+begin_src conf :tangle ~/.config/waybar-crypto/config.ini
[general]
currency = usd
currency_symbol = $
display = price,percent_change_7d
spacer_symbol = |

format_percent_change_7d = {val:+.{dp}f}%
...
#+begin_src vimrc :tangle ~/.config/nvim/init.vim
set background=dark
source /usr/share/nvim/runtime/colors/catppuccin-macchiato.lua
#+end_src

* yt-dlp
#+begin_src conf :tangle ~/.config/yt-dlp/config
-o "%(title)s [%(webpage_url)s].%(ext)s"
#+end_src
...
only = [ "system", "firmware", "cargo", "go", "bun_packages", "pnpm", "pipx", "custom_commands"]
...
* Emacsclient Mail Helper
#+begin_src desktop :tangle ~/.local/share/applications/emacsclient-mail-helper.desktop
[Desktop Entry]
Categories=Network;Email;
Comment=GNU Emacs is an extensible, customizable text editor - and more
Exec=emacsclient_mail_helper %u
Icon=emacs
...
* mpv Cut
#+begin_src lua :tangle ~/.config/mpv-cut/config.lua
...
ACTIONS.ENCODE = function(d)
	local args = {
		...
		"-c:v", "h264_qsv",
		"-global_quality", "25",
		utils.join_path(d.indir, "ENCODE_" .. d.channel .. "_" .. d.infile_noext .. "_FROM_" .. d.start_time_hms .. "_TO_" .. d.end_time_hms .. ".mp4")
	}
...
* Lid Action
#+begin_src conf-toml :tangle /doas::/etc/systemd/logind.conf
[Login]
...
swhks &
sleep 5
doas swhkd -c ~/.config/swhkd/swhkdrc &
...
footclient $SHELL -c "mgitstatus -f -e --flatten -d 1 ~/.password-store; mgitstatus -f -e --flatten ~/Files; $SHELL" &
...
awww img --transition-fps 60 --transition-bezier 0.25,1,0.5,1 --transition-duration 0.4  ~/Files/wallpapers/16-9/$(ls ~/Files/wallpapers/16-9 | shuf -n 1)  -t $direction
#+end_src
...
wl-paste | while IFS= read -r line; do
	if [ -n "$line" ]; then
		$BROWSER "$(echo "$line" | awk '{print $2}' | awk -F'\\[\\[|\\]\\[|\\]\\]' '{print $2}')"
	fi
done
#+end_src
...
** Extract Youtube URL
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/extract_youtube_url
filename="$1"
...
echo "yt-dlp -x -o \"$title [%(webpage_url)s].%(ext)s\" \"$url\""
#+end_src

** Emacsclient Mail Helper
#+begin_src sh :shebang "#! /usr/bin/env sh" :tangle ~/.local/bin/emacsclient_mail_helper
...
** rehandaphedar Backup Data
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_backup_data
export PASSPHRASE="$(pass init/backup/home)" 

duplicity ~/Files/audio sftp://rehandaphedar@localhost:8008/Files/audio
duplicity ~/Files/backups sftp://rehandaphedar@localhost:8008/Files/backups
duplicity ~/Files/calls sftp://rehandaphedar@localhost:8008/Files/calls
duplicity ~/Files/dcim sftp://rehandaphedar@localhost:8008/Files/dcim
duplicity ~/Files/documents sftp://rehandaphedar@localhost:8008/Files/documents
duplicity ~/Files/emojis sftp://rehandaphedar@localhost:8008/Files/emojis
duplicity ~/Files/finance sftp://rehandaphedar@localhost:8008/Files/finance
duplicity ~/Files/media sftp://rehandaphedar@localhost:8008/Files/media
duplicity ~/Files/pdfs sftp://rehandaphedar@localhost:8008/Files/pdfs
duplicity ~/Files/roam sftp://rehandaphedar@localhost:8008/Files/roam
duplicity ~/Files/mbbs sftp://rehandaphedar@localhost:8008/Files/mbbs
duplicity ~/Files/wallpapers sftp://rehandaphedar@localhost:8008/Files/wallpapers
#+end_src

** rehandaphedar Fabric Run Pattern
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_fabric_run_pattern
pattern="$(fabric -l | wofi -dp 'Pattern: ')"
if [ ! -z "${pattern}" ];
then
	wl-paste | fabric --pattern "$pattern"
fi
#+end_src

** rehandaphedar Fabric Print Pattern
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_fabric_print_pattern
pattern="$(fabric -l | wofi -dp 'Pattern: ')"
if [ ! -z "${pattern}" ];
then
	wl-paste | fabric --pattern "$pattern" --dry-run
fi
#+end_src

** rehandaphedar Fabric Print YouTube Transcript
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_fabric_print_youtube_transcript
youtube_transcript_api --format text "$(wl-paste)" | fabric --pattern "clean_text"
#+end_src

** rehandaphedar Fabric Run YouTube Transcript Pattern
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_fabric_run_youtube_transcript_pattern
pattern="$(fabric -l | wofi -dp 'Pattern: ')"
if [ ! -z "${pattern}" ];
then
	rehandaphedar_fabric_print_youtube_transcript | fabric --pattern "$pattern"
fi
#+end_src

** rehandaphedar Fabric Print YouTube Transcript Pattern
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/rehandaphedar_fabric_print_youtube_transcript_pattern
pattern="$(fabric -l | wofi -dp 'Pattern: ')"
if [ ! -z "${pattern}" ];
then
	rehandaphedar_fabric_print_youtube_transcript | fabric --pattern "$pattern" --dry-run
fi
#+end_src

** Vid Length
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/vid_length
duration=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$1")
printf '%02d:%02d:%02d\n' $((${duration%.*}/3600)) $((${duration%.*}%3600/60)) $((${duration%.*}%60))
#+end_src

** Vid Length All
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/vid_length_all
format_duration() {
  local seconds=${1%.*}
  printf '%02d:%02d:%02d' \
    $((seconds/3600)) \
...
output_csv_row() {
  local file=$1
  local duration=$2
  jq -rn --arg f "${file:0:120}" --arg t "$duration" '[$f,$t]|@csv'
}
{
echo "File,Duration"
...
for f in $1; do
  ...
  dur=$(get_duration "$f") || continue
  [[ "$dur" == "N/A" ]] && continue
  
  formatted_dur=$(format_duration "$dur")
  output_csv_row "$f" "$formatted_dur"
  
  total=$(awk "BEGIN {print $total + $dur}")
done

formatted_total=$(format_duration "$total")
output_csv_row "Total" "$formatted_total"
} | gum table --print
#+end_src

** Instaify
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/instaify
for filepath in "$@"; do
    dir=$(dirname "$filepath")
    base=$(basename "$filepath")
    name="${base%.*}"
    ext="${base##*.}"
	output="$dir/${name}_insta.$ext"
	
    magick "$filepath" -resize "1080x1080" -gravity center -background white -extent 1080x1080 "$output"
done
#+end_src

** YTify
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/ytify
ext=".mp4"
base="$(basename "$1" ".mp4")"

ffmpeg -i "$1" -t 00:14:30 -c copy "${base}_youtube${ext}"
(printf 'This video has been truncated due to YouTube upload length limits. The full video is available on other platforms at rehandaphedar.com.\n\n'; cat description.md) | tee /dev/tty | wl-copy
#+end_src

** Get Links Content
#+begin_src bash :shebang "#! /usr/bin/env bash" :tangle ~/.local/bin/get_links_content
wl-paste | while IFS= read -r line; do
    curl -s "$line" | html2text
done
#+end_src
